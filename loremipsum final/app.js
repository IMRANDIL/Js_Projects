...
let randomtxt = text[Math.floor(Math.random()*text.length)];


// input value < 0 or equal to 0;
// input value >9;
if(isNaN(values) || values <= 0 || values > 9){

    loremtext.innerHTML = `<p class='result'>${randomtxt}</p>`;

}
else{
  let inputnumber = text.slice(0, values);

  let inputtext = inputnumber.map((txtt) =>{
    return `<p class = 'result'>${txtt}</p>`;
    
  }).join('');

  loremtext.innerHTML = inputtext;
